...
class Encoder4Editing(nn.Module):
    ...
    def forward(self, x):
        x = self.input_layer(x) # [1, 64, 256, 256]
        ...
        w0 = self.styles[0](c3) # [1, 512]
        w = w0.repeat(self.style_count, 1, 1).permute(1, 0, 2) # [1, 18, 512]
        ...
        for i in range(1, self.style_count):  # Infer additional deltas
            if i == self.coarse_ind:
                ...
                features = p1
            delta_i = self.styles[i](features) # [1, 512]
            w[:, i] += delta_i # [1, 18, 512]
...
class E4E_Inversion(nn.Module):
    ...
    def forward(self, reals):
        ...
        w = w + self.latent_avg
        highres_outs = {f"{self.out_res}x{self.out_res}": c} # [1, 128, 64, 64]
