...
import random
import sys
sys.path.append('/home/zhr/test/Rec/models/rec_models')
from models.rec_models import sp_encoder, gl_encoder, discriminator
from models.rec_models import generator_all_level
...
class RecModel(nn.Module):
    def __init__(self, args):
        ...
        self.G = generator_all_level.StyleGAN2ResnetGenerator()
    ...
    def compute_discriminator_losses(self, real, E2, E3, E4, E5, G2, G3, G4, G5):
        ...
        rec = self.G(sp, gl, sp_style, skip, skip2, skip3, skip5, G2, G3, G4, G5) # [8, 3, 256, 256]
    ...
    def compute_generator_losses(self, real, E2, E3, E4, E5, G2, G3, G4, G5):
        losses = {}
        ...
        skip5 = E5(image_edge)
        gl, sp_style = self.E_GL(delta_edit) # [8, 2048] [8, 16, 16, 16]
        rec = self.G(sp, gl, sp_style, skip, skip2, skip3, skip5, G2, G3, G4, G5)
        #rec = torch.nn.functional.normalize(rec, dim=1)

        losses["G_CE"] = self.l1_loss(rec, ori_imag) * self.lambda_L1 #L1
        losses["G_DICE"] = self.lpips_loss(rec, ori_imag) * self.lambda_Lpips # Lpips
        ...
        losses["G_rec"] = loss.gan_loss(
            self.D(rec),
            should_be_classified_as_real=True
        ) * (self.lambda_GAN * 0.5)
        losses["G_rec"] = losses["G_rec"].mean() # G_GAN_rec
