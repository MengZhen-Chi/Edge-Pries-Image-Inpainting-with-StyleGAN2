...
from .base_network_256 import BaseNetwork
...
class StyleGAN2ResnetGenerator(BaseNetwork):
    ...
    def __init__(self):
        ...
        for j in range(self.netE_num_downsampling_sp):
            out_channel = self.nf(j + 1) # 512、256、128
            layer_name = "UpsamplingResBlock%d" % (2 ** (4 + j)) # UpsamplingResBlock16、UpsamplingResBlock32、UpsamplingResBlock64
            new_layer = UpsamplingResnetBlock(
                in_channel, out_channel, self.global_code_ch, self.spatial_code_ch,
                blur_kernel) # 512-512、512-256、256-128
            self.add_module(layer_name, new_layer)
            in_channel = out_channel
    ...
    def forward(self, spatial_code, global_code, sp_style, skip, skip2, skip32, G2, G4, G5):
        ...
        upsampling_layer = getattr(self, "UpsamplingResBlock16")
        x, sp_style = upsampling_layer(x, global_code, sp_style) # [8, 512, 32, 32] [8, 16, 32, 32]
        #padding = (8, 8, 8, 8)  # (left, right, top, bottom)
        #x = F.pad(x, pad=padding, mode='constant', value=0)  # (B, C, 32, 32)
        x = torch.cat((x, skip32), dim=1) # [8, 1280, 32, 32]
        x = G5(x, global_code)
        ...
        for j in range(self.netE_num_downsampling_sp):
            ...
            x, sp_style = upsampling_layer(x, global_code, sp_style) # [8, 512, 32, 32] [8, 16, 32, 32]
            if j == 1:
                x = torch.cat((x, skip2), dim=1) # [8, 320, 128, 128]
                x = G2(x, global_code)
